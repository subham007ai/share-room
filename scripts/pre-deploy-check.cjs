...
// Allow either ANON_KEY or PUBLISHABLE_KEY
const supabaseUrl = envVars['VITE_SUPABASE_URL'] || process.env['VITE_SUPABASE_URL'];
const supabaseKey = envVars['VITE_SUPABASE_ANON_KEY'] || envVars['VITE_SUPABASE_PUBLISHABLE_KEY'] || process.env['VITE_SUPABASE_ANON_KEY'] || process.env['VITE_SUPABASE_PUBLISHABLE_KEY'];

if (!supabaseUrl) {
    console.log(`${colors.red}✗ Missing VITE_SUPABASE_URL${colors.reset}`);
    hasError = true;
} else {
...
if (!supabaseKey) {
    console.log(`${colors.red}✗ Missing VITE_SUPABASE_ANON_KEY or VITE_SUPABASE_PUBLISHABLE_KEY${colors.reset}`);
    hasError = true;
} else {
    console.log(`${colors.green}✓ VITE_SUPABASE_KEY found${colors.reset}`);
}
...
if (supabaseUrl) {
    try {
        ...
        console.log(`${colors.red}✗ Invalid Supabase URL format: ${supabaseUrl}${colors.reset}`);
        hasError = true;
        finish();
    }
} else {
